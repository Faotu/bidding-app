...
const DisplayMyGroups: React.FC<{
  featured: boolean;
}> = ({ featured }) => {
  const user = useUserSession();
  const [filters, setFilters] = useState<FilterType[]>([
    {
      field: `members.${user?.data?.userId}`,
      op: 'in',
      value: ['owner', 'member'],
    },
  ]);
  ...
  const { page, setPage, itemsPerPage } = usePagination({ itemsPerPage: 10 });
...
export default DisplayMyGroups;
