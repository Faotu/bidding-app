...
import { UserProfessionalData, UserProfileData } from '~/core/session/types/user-data';
import Alert from '~/core/ui/Alert';
import PageLoadingIndicator from '~/core/ui/PageLoadingIndicator';

interface CompleteOnboardingStepData extends UserProfileData, UserProfessionalData {
  organization: string;
  availability: string,
}
  ...
  if (error) {
    return (
      <div className={'flex flex-col space-y-4'}>
        <Alert type={'error'}>{error ? "Failed to save" : ""}</Alert>
      </div>
    );
  }
